...
def _build_experiment_specs(args: argparse.Namespace) -> list[ExperimentSpec]:
    ...
    # 实验矩阵的小幅工程优化：
    # 1. 论文明确给出的：PPO 可视为 sigma=0；
    # 2. 当前仓库已有实现：algo=ppo 时会忽略非零 sigma；
    # 3. 因此 suite 层不再生成「ppo + 非零 sigma」的无效重复组合，
    #    只保留 ppo+0，而 eppo 继续展开用户传入的 sigma 列表。
    for algo in args.algos:
        if str(algo) == "ppo":
            sigma_candidates = [0.0]
        else:
            sigma_candidates = [float(sigma) for sigma in args.sigmas]

        for sigma, seed in itertools.product(sigma_candidates, args.seeds):
            experiment_id = (
